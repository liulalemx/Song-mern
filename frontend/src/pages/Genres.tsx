  ...
  return (
    <Container>
      {Object.keys(stats).length ? (
        <div>
          <h1 className="p-4 sm:px-6 lg:px-8 text-2xl font-bold">Genres</h1>
          <div className="flex flex-col gap-y-8 px-4 sm:px-6 lg:px-8">
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {stats.genres.data.map((genre, i) => (
                  <div
                    key={i}
                      ...
                      <CardContent className="pt-4">
                        <div className="flex items-center justify-center relative bg-foreground/5 dark:bg-background rounded-lg transition all duration-300 hover:scale-105">
                          <p className=" aspect-square object-cover rounded-lg  text-5xl">
                            {genre._id.charAt(0).toUpperCase()}
                          </p>
                        </div>
                      </CardContent>
                      <CardFooter className="flex-col items-start">
                        <div>
                          <p className="font-semibold text-lg">{genre._id}</p>
